...
```bash
python3 cdp_capture.py --list_pages
```
...
```bash
python3 cdp_capture.py --list_pages
python3 cdp_capture.py --page_hint example.com/app --capture both --reload
```
...
- `--page_hint`: keyword used to match page URL or title
...
- `--no_raw`: do not write raw JSONL files
- `--print_match`: print a preview for matched records
...
- `--no_raw`: raw files are not written
