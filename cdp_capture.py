...
class MatchRule:
    def __init__(self, raw: str):
        ...
        self.all_patterns = []
        if not self.is_regex and "&&" in self.pattern:
            self.all_patterns = [part.strip() for part in self.pattern.split("&&") if part.strip()]

    def matches(self, text: str) -> bool:
        if self.regex is not None:
            return bool(self.regex.search(text))
        if self.all_patterns:
            return all(part in text for part in self.all_patterns)
    ...
    def call(self, method: str, params: Optional[dict] = None) -> dict:
        ...
        while True:
            ...
            if "method" in message:
                message["_recv_ts"] = received_at
                if message["method"] == "Page.javascriptDialogOpening":
                    dialog = message.get("params") or {}
    ...
    if page_hint:
        for page in pages:
            ...
            if page_hint in url or page_hint in title:
                return page
        raise RuntimeError(
            f"No page matched page_hint={page_hint!r}; check the keyword or use --page_id"
        )

    raise RuntimeError("Please provide --page_id or --page_hint to select a target page")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cdp_base", default=DEFAULT_CDP_BASE, help="CDP base URL")
    parser.add_argument(
        "--capture",
    ...
    parser.add_argument("--page_id", help="Target page id")
    parser.add_argument(
        "--page_hint",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="WebSocket payload match rule, repeatable; rules are ORed; use && inside one rule for AND; use re: prefix for regex",
    )
    parser.add_argument(
        "--http_match",
        action="append",
        default=[],
        help="HTTP response body match rule, repeatable; rules are ORed; use && inside one rule for AND; use re: prefix for regex",
    )
    parser.add_argument(
        "--output_dir",
...
def matches_ws(
    ...
    if ws_url and ws_url_rules and not any(rule.matches(ws_url) for rule in ws_url_rules):
        return False
...
def build_ws_frame_context(event: dict, runtime: CaptureRuntime) -> WsFrameContext:
    ...
    return WsFrameContext(
        ...
        event_received_at=str(event.get("_recv_ts") or ""),
    )


def build_ws_record(runtime: CaptureRuntime, context: WsFrameContext) -> dict:
    return {
        "ts": context.event_received_at or now_iso(),
        "protocol": "ws",
...
def build_http_record(runtime: CaptureRuntime, context: HttpResponseContext, params: dict, event: dict) -> dict:
    entry = context.entry
    return {
        "ts": str(event.get("_recv_ts") or now_iso()),
        "protocol": "http",
        "page_id": runtime.page.get("id"),
        "page_title": runtime.page.get("title"),
        "page_url": runtime.page.get("url"),
...
def handle_http_event(event: dict, runtime: CaptureRuntime) -> bool:
    ...
    record = build_http_record(runtime, context, params, event)
...
def enable_capture(runtime: CaptureRuntime) -> None:
    runtime.client.call("Page.enable")
...
def process_event(runtime: CaptureRuntime, event: dict) -> None:
    if event.get("method") == "Page.javascriptDialogOpening":
        ...
        if params.get("type") == "beforeunload":
            log_action("auto-accept beforeunload dialog")
            time.sleep(1)
            runtime.client.send("Page.handleJavaScriptDialog", {"accept": True})
        return
...
def reconnect_runtime(runtime: CaptureRuntime) -> None:
    runtime.page = choose_target(
        runtime.config.cdp_base,
        runtime.config.page_id,
        runtime.config.page_hint,
    )
    ws_debugger_url = runtime.page.get("webSocketDebuggerUrl")
    if not ws_debugger_url:
        raise RuntimeError("target page does not expose webSocketDebuggerUrl; cannot reconnect to CDP")

    runtime.client.close()
    runtime.client = CdpClient(ws_debugger_url)
    runtime.ws_state = WsCaptureState()
    runtime.http_state = HttpCaptureState()
    runtime.last_status_at = time.monotonic()
    runtime.last_status_snapshot = (-1, -1, -1, -1, -1, -1)


def run_capture_with_retries(runtime: CaptureRuntime) -> None:
    attempt = 0
    max_attempts = RECONNECT_RETRIES
    unlimited_retries = max_attempts < 0
    ...
    while True:
        try:
            ...
            if not unlimited_retries and attempt >= max_attempts:
                log_done("cdp websocket connection was closed", level=logging.WARNING)
                return

            attempt += 1
            log_done(
                "cdp websocket connection was closed, reconnecting attempt=%s/%s after %.1fs",
                attempt,
                "infinite" if unlimited_retries else max_attempts,
            ...
            try:
                reconnect_runtime(runtime)
            except Exception as exc:
                log_done(
                    "cdp reconnect attempt=%s failed err=%s",
                    attempt,
                    exc,
                    level=logging.WARNING,
                )
            ...
            if not unlimited_retries and attempt >= max_attempts:
                log_done("cdp reconnect failed err=%s", exc, level=logging.WARNING)
                return

            attempt += 1
            log_done(
                "cdp connection error err=%s reconnecting attempt=%s/%s after %.1fs",
                exc,
                attempt,
                "infinite" if unlimited_retries else max_attempts,
                RECONNECT_DELAY_SECONDS,
...
def main() -> int:
    ...
    try:
        try:
            run_capture_with_retries(runtime)
